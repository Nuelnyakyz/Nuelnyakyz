# Hi there 👋 I'm Emmanue

A passionate **Full-Stack Developer** with expertise in building scalable applications across multiple platforms. I love solving complex problems and creating elegant solutions with clean, maintainable code.

---

## 🚀 About Me

I'm a developer who thrives on turning ideas into reality. With experience in **backend systems**, **mobile development**, **web applications**, and **data processing**, I bring a versatile skill set to every project. I'm committed to continuous learning and staying updated with modern development practices.

- 💼 **Full-Stack Development** - Web, Mobile, Backend
- 🔧 **Problem Solver** - Building efficient and scalable solutions
- 📚 **Always Learning** - Exploring new technologies and best practices
- 🤝 **Collaborative** - Love working with teams to achieve great results

---

## 💻 Tech Stack

### Languages
![Go](https://img.shields.io/badge/Go-00ADD8?style=flat&logo=go&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Kotlin](https://img.shields.io/badge/Kotlin-7F52FF?style=flat&logo=kotlin&logoColor=white)
![PHP](https://img.shields.io/badge/PHP-777BB4?style=flat&logo=php&logoColor=white)
![HTML/CSS](https://img.shields.io/badge/HTML%2FCSS-E34C26?style=flat&logo=html5&logoColor=white)

### Frameworks & Tools
- **Backend**: Go, Laravel, Node.js
- **Frontend**: TypeScript, React (implied from projects)
- **Mobile**: Kotlin (Android)
- **Database**: SQL, NoSQL
- **Tools**: Git, Docker, REST APIs

---

## 📁 Featured Projects

### 🔙 Backend Development
| Project | Description | Tech |
|---------|-------------|------|
| [306-go-backend](https://github.com/Nuelnyakyz/306-go-backend) | Backend service built with Go | Go |
| [MillieW](https://github.com/Nuelnyakyz/MillieW) | Sky Cash Aviator backend | Go |
| [PaymentsApp](https://github.com/Nuelnyakyz/PaymentsApp) | Payment gateway system | Laravel/PHP |

### 📱 Mobile Development
| Project | Description | Tech |
|---------|-------------|------|
| [notesAppV2](https://github.com/Nuelnyakyz/notesAppV2) | Notes application | Kotlin |

### 🌐 Web Applications
| Project | Description | Tech |
|---------|-------------|------|
| [Mannu](https://github.com/Nuelnyakyz/Mannu) | Web application | TypeScript |
| [CourseIt](https://github.com/Nuelnyakyz/CourseIt) | Course management system | CSS/HTML |
| [Student-Task-Manager](https://github.com/Nuelnyakyz/Student-Task-Manager) | Task management tool | TypeScript |

### 🛠️ Specialized Projects
| Project | Description | Tech |
|---------|-------------|------|
| [pdf_reader](https://github.com/Nuelnyakyz/pdf_reader) | PDF to Audio converter | Python |
| [sentin-gantt](https://github.com/Nuelnyakyz/sentin-gantt) | Gantt chart visualization | HTML/JavaScript |
| [SentinAIPresentation](https://github.com/Nuelnyakyz/SentinAIPresentation) | AI presentation tool | HTML/JavaScript |
| [simple-uok-uon-lms](https://github.com/Nuelnyakyz/simple-uok-uon-lms) | Learning Management System | HTML |

---

## 🎯 What I'm Good At

✅ Building **RESTful APIs** and backend services  
✅ Developing **responsive web applications**  
✅ Creating **mobile applications** for Android  
✅ Database design and optimization  
✅ Code quality and best practices  
✅ Problem-solving and debugging  
✅ Working in agile environments  

---

## 📊 My Approach

- 🎨 **Clean Code** - Writing readable, maintainable code
- ⚡ **Performance** - Optimizing for speed and efficiency
- 🧪 **Testing** - Ensuring reliability through proper testing
- 📖 **Documentation** - Clear and comprehensive documentation
- 🔄 **Continuous Improvement** - Always refining and learning

---

## 🌟 Highlights

- Full-stack development across multiple platforms
- Experience with both frontend and backend systems
- Proven ability to work with diverse technology stacks
- Strong foundation in software engineering principles
- Portfolio of real-world applications and solutions

---

## 📫 Let's Connect

I'm always interested in collaborating on exciting projects or discussing new ideas. Feel free to reach out!

- 🔗 [GitHub](https://github.com/Nuelnyakyz)
- 💬 Open to discussions and collaboration

---

## 📈 Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Nuelnyakyz&show_icons=true&theme=radical)

---

## 💡 Philosophy

> "Code is poetry written for machines but read by humans. Write it with care, clarity, and creativity."

Every project is an opportunity to grow, learn, and create something meaningful. I believe in building solutions that are not just functional but also elegant and maintainable.

---

*Last updated: May 2026*
